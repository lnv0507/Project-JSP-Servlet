<%@ page contentType="text/html;charset=UTF-8" language="java" %>
    <div class="row">
        <div class="col-md-8 project-images">
            <img src="images/portfolio/view/NO2-3.jpg" alt="" class="img-responsive" />
            <img src="images/portfolio/view/NO2-2.jpg" alt="" class="img-responsive" />
        </div>
        <div class="col-md-4">
            <div class="project-info">
                <h2>Thiết kế</h2>

                <div class="details">
                    <div class="info-text">
                        <span class="title">Mã thiết kế</span>
                        <span class="val">H 2</span>
                    </div>

                    <div class="info-text">
                        <span class="title">Loại thiết kế</span>
                        <span class="val">Nhà ở</span>
                    </div>

                    <div class="info-text">
                        <span class="title">Tổng chi phí</span>
                        <span class="val">$40.000</span>
                    </div>

<!--                    <div class="info-text">-->
<!--                        <span class="title">Client</span>-->
<!--                        <span class="val">Four Seasons</span>-->
<!--                    </div>-->

<!--                    <div class="info-text">-->
<!--                        <span class="title">Category</span>-->
<!--                        <span class="val">Hospitaly</span>-->
<!--                    </div>-->
<!--                </div>-->

                <p></p>

                <p>
                    <h4>Mô tả</h4>
                    Với lối thiết kế trung thành với tone chủ đạo vẫn là màu trắng kết hợp với màu hồng nhạt được xem là sự kết hợp kinh điển trong thiết kế nội thất. Sắc hồng là màu sắc tạo sự nhẹ nhàng và màu trắng là màu trung gian, căn phòng sẽ trở nên đầy tính nghệ thuật và êm ái nhưng không kém phần quyến rũ tác động mạnh mẽ vào thị giác, tạo nên dự hấp dẫn riêng cho căn hộ.
                </p>
                    <h4>Các sản phẩm :</h4>
                    <ul class="products-design">
                        <li><a href="product-details.jsp">Giường ngủ GN01 </a> </li> Số lượng : 1
                        <li><a href="product-details.jsp">Ghế tựa hồng </a> </li>Số lượng : 4
                        <li><a href="product-details.jsp">Tủ gỗ sim </a> </li>Số lượng : 5
                        <li><a href="product-details.jsp">Sofa GN200 </a> </li>Số lượng : 2
                        <li><a href="product-details.jsp">Bàn lớn A20 </a> </li>Số lượng : 1
                    </ul>
                </p>


            </div>
        </div>
    </div>
</div>
